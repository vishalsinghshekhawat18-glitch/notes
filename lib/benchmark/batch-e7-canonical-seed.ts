  ...
  },
  {
    ...
    "contentBlocks": [
      ...
      },
      {
        ...
        "body": "### 1. Solow Capital Accumulation & Steady-State\\nLet $k = K / (AL)$ be capital per effective worker, $s$ be the savings rate, $\\\\delta$ be depreciation, $n$ be population growth rate, and $g_A$ be the rate of technological progress.\\n- **Steady-State Condition ($k^*$)**: Occurs when new capital investment equals break-even replacement investment:\\n$$s \\\\cdot f(k^*) = (n + g_A + \\\\delta)k^*$$\\n- *Exam Implication*: An increase in the savings rate ($s$) shifts the investment line upward, raising the *level* of capital per worker ($k^*$) and output per worker ($y^*$). However, once the new steady state is reached, the long-run *growth rate* of per capita output returns strictly to the rate of technological progress ($g_A$).\\n\\n### 2. The Golden Rule Level of Capital ($k_{\\\\text{gold}}$)\\nThe Golden Rule identifies the specific steady state that maximizes sustainable **consumption per worker** ($c^*$):\\n$$\\\\mathbf{MP_K = n + g_A + \\\\delta}$$\\n- **Economic Meaning**: At the Golden Rule capital stock, the Marginal Product of Capital ($MP_K$) exactly matches the break-even investment requirement. If an economy accumulates capital beyond this point ($k^* > k_{\\\\text{gold}}$), it suffers from **Dynamic Inefficiency** (it saves too much, leaving less output available for consumption).\\n\\n### 3. Incremental Capital-Output Ratio (ICOR)\\n$$\\\\text{ICOR} = \\\\frac{\\\\Delta K}{\\\\Delta Y} = \\\\frac{\\\\text{Gross Fixed Capital Formation (\\\\% of GDP)}}{\\\\text{GDP Growth Rate (\\\\%)}}$$\\n- *Interpretation*: If India invests 32% of GDP into capital formation and achieves 8% real GDP growth, its $\\\\text{ICOR} = 32 / 8 = 4.0$. A **lower ICOR reflects higher capital productivity and efficiency**.",
        "order": 3
      },
      {
        "type": "EXAM_APPLICATION",
