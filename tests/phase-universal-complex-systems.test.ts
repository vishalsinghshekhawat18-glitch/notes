...
import { MODULE_U3_CONCEPTS, seedModuleU3LivingEarthComplexSystems } from '../lib/benchmark/batch-u3-complex-systems-seed';

describe('Universal Knowledge & World Understanding — Module U3: The Living Earth, Human Cognition & Complex Systems', () => {
  beforeAll(async () => {
    await seedModuleU3LivingEarthComplexSystems();
  });

  describe('Structural & Quantitative Verification', () => {
    it('should contain exactly 5 canonical concepts under Topic 1', () => {
      expect(MODULE_U3_CONCEPTS.length).toBe(5);

      const topicOrders = new Set(MODULE_U3_CONCEPTS.map((c) => c.topicOrder));
      expect(topicOrders.size).toBe(1);
      expect(topicOrders.has(1)).toBe(true);

      const expectedIds = ['CON-EARTH-01', 'CON-EARTH-02', 'CON-EARTH-03', 'CON-EARTH-04', 'CON-EARTH-05'];
      const actualIds = MODULE_U3_CONCEPTS.map((c) => c.id);
      expect(actualIds).toEqual(expectedIds);

      const slugs = new Set(MODULE_U3_CONCEPTS.map((c) => c.slug));
      expect(slugs.size).toBe(5);
    });

    it('should enforce zero exam mappings (pure universal knowledge track)', () => {
      for (const concept of MODULE_U3_CONCEPTS) {
  ...
  describe('Database Persistence & Relationship Verification', () => {
    it('should verify that all 5 concepts exist under the World Understanding domain in SQLite', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'the-living-earth-human-cognition-complex-systems' },
        include: {
          domain: true,
          topics: {
            include: {
      ...
      for (const concept of allConcepts) {
        ...
        for (const cl of concept.claims) {
          ...
          expect(cl.evidence[0].sourceId).toBe('SRC-LIVING-EARTH-COMPLEX-SYSTEMS');
        }
    ...
    it('should verify total claims, blocks, units, and questions count in database for Module U3', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'the-living-earth-human-cognition-complex-systems' },
      });
    ...
    it('should confirm strict isolation: universal complex systems concepts do not leak into exam lenses', async () => {
      const examMappings = await db.examConceptMapping.findMany({
        where: {
          concept: {
            id: {
              in: ['CON-EARTH-01', 'CON-EARTH-02', 'CON-EARTH-03', 'CON-EARTH-04', 'CON-EARTH-05'],
            },
          },
        },
      });
