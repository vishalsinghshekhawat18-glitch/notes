...
import { MODULE_U2_CONCEPTS, seedModuleU2ModernTechnology } from '../lib/benchmark/batch-u2-modern-technology-seed';

describe('Universal Knowledge & World Understanding — Module U2: How Modern Technology Actually Works', () => {
  beforeAll(async () => {
    await seedModuleU2ModernTechnology();
  });

  describe('Structural & Quantitative Verification', () => {
    it('should contain exactly 5 canonical concepts under Topic 1', () => {
      expect(MODULE_U2_CONCEPTS.length).toBe(5);

      const topicOrders = new Set(MODULE_U2_CONCEPTS.map((c) => c.topicOrder));
      expect(topicOrders.size).toBe(1);
      expect(topicOrders.has(1)).toBe(true);

      const expectedIds = ['CON-TECH-01', 'CON-TECH-02', 'CON-TECH-03', 'CON-TECH-04', 'CON-TECH-05'];
      const actualIds = MODULE_U2_CONCEPTS.map((c) => c.id);
      expect(actualIds).toEqual(expectedIds);

      const slugs = new Set(MODULE_U2_CONCEPTS.map((c) => c.slug));
      expect(slugs.size).toBe(5);
    });

    it('should enforce zero exam mappings (pure universal knowledge track)', () => {
      for (const concept of MODULE_U2_CONCEPTS) {
        expect(concept.examMappings).toEqual([]);
      }
    });

    it('should verify each concept has at least 4 claims with authoritative technological locators', () => {
      let totalClaims = 0;
      for (const concept of MODULE_U2_CONCEPTS) {
  ...
  describe('Database Persistence & Isolation Verification', () => {
    it('should verify that all 5 concepts exist under the World Understanding domain in SQLite', async () => {
      ...
      for (const concept of allConcepts) {
        ...
        for (const cl of concept.claims) {
          ...
          expect(cl.evidence[0].sourceId).toBe('SRC-MODERN-TECH-FOUNDATIONS');
        }
    ...
    it('should verify total claims, blocks, units, and questions count in database', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'foundations-of-modern-technology' },
      });
    ...
    it('should confirm strict isolation: universal tech concepts do not leak into exam lenses', async () => {
      const examMappings = await db.examConceptMapping.findMany({
        where: {
          concept: {
            id: {
              in: ['CON-TECH-01', 'CON-TECH-02', 'CON-TECH-03', 'CON-TECH-04', 'CON-TECH-05'],
            },
          },
        },
      });
