...
import { BATCH_M1_CONCEPTS, seedBatchM1MarxCapitalKnowledge } from '../lib/benchmark/batch-m1-marx-capital-seed';

describe('Marx Capital Volume 1 — Batch M1: Commodities, Value-Form, Fetishism & The Transformation of Money into Capital', () => {
  beforeAll(async () => {
    await seedBatchM1MarxCapitalKnowledge();
  });

  describe('Structural & Quantitative Verification', () => {
    it('should contain exactly 5 canonical concepts across Topics 1 and 2', () => {
      expect(BATCH_M1_CONCEPTS.length).toBe(5);

      const topicOrders = new Set(BATCH_M1_CONCEPTS.map((c) => c.topicOrder));
      expect(topicOrders.size).toBe(2);
      expect(topicOrders.has(1)).toBe(true);
      expect(topicOrders.has(2)).toBe(true);

      const expectedIds = ['CON-MARX-01', 'CON-MARX-02', 'CON-MARX-03', 'CON-MARX-04', 'CON-MARX-05'];
      const actualIds = BATCH_M1_CONCEPTS.map((c) => c.id);
      expect(actualIds).toEqual(expectedIds);

      // Verify unique slugs
      const slugs = new Set(BATCH_M1_CONCEPTS.map((c) => c.slug));
      expect(slugs.size).toBe(5);
    });

    it('should verify each concept has at least 4 claims with authoritative lecture locators', () => {
      let totalClaims = 0;
      for (const concept of BATCH_M1_CONCEPTS) {
        ...
        for (const claim of concept.claims) {
          ...
          expect(claim.locator).toContain("Marx's Capital Vol 1 with Paul North");
    ...
    it('should verify each concept maps to all 3 target examinations', () => {
      const requiredExams = ['SOCIOLOGY_OPTIONAL', 'PHILOSOPHY_OPTIONAL', 'ACADEMIC_PHILOSOPHY'];
      let totalMappings = 0;
      for (const concept of BATCH_M1_CONCEPTS) {
    ...
    it('should verify each concept has at least 3 graded questions with 4 options and trap explanations', () => {
  ...
  describe('Database Persistence Verification', () => {
    it('should verify Domain and Subject records exist in SQLite dev.db', async () => {
      const domain = await db.domain.findFirst({ where: { slug: 'economics' } });
      expect(domain).toBeDefined();

      const subject = await db.subject.findFirst({ where: { slug: 'marxian-political-economy-capital-volume-1' } });
      expect(subject).toBeDefined();
      expect(subject?.domainId).toBe(domain?.id);
    });

    it('should verify Topics 1 and 2 exist under marxian-political-economy-capital-volume-1', async () => {
      const subject = await db.subject.findFirst({ where: { slug: 'marxian-political-economy-capital-volume-1' } });
      ...
      expect(topics[0].slug).toBe('the-commodity-dual-nature-value-form-and-fetishism');
      expect(topics[1].slug).toBe('transformation-of-money-into-capital-and-labor-power');
    });

    it('should verify all 5 Concepts exist in database with child relations', async () => {
      for (const cDef of BATCH_M1_CONCEPTS) {
        ...
        // Verify evidence links to SRC-YALE-MARX-CAPITAL-1
        for (const cl of concept!.claims) {
          expect(cl.evidence.length).toBeGreaterThanOrEqual(1);
          expect(cl.evidence[0].sourceId).toBe('SRC-YALE-MARX-CAPITAL-1');
        }
      }
    });
    ...
    it('should verify exams were registered and linked correctly', async () => {
      const exams = await db.exam.findMany({
        where: {
          slug: {
            in: ['sociology-optional', 'philosophy-optional', 'academic-philosophy'],
          },
        },
      });
