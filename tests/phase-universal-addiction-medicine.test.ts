...
import { MODULE_U4_CONCEPTS, seedModuleU4AddictionMedicine } from '../lib/benchmark/batch-u4-addiction-medicine-seed';

describe('Universal Knowledge & World Understanding — Module U4: Addiction Treatment & Clinical Neurobiology', () => {
  beforeAll(async () => {
    await seedModuleU4AddictionMedicine();
  });

  describe('Structural & Quantitative Verification', () => {
    it('should contain exactly 5 canonical concepts under Topic 1', () => {
      expect(MODULE_U4_CONCEPTS.length).toBe(5);

      const topicOrders = new Set(MODULE_U4_CONCEPTS.map((c) => c.topicOrder));
      expect(topicOrders.size).toBe(1);
      expect(topicOrders.has(1)).toBe(true);

      const expectedIds = ['CON-ADDICT-01', 'CON-ADDICT-02', 'CON-ADDICT-03', 'CON-ADDICT-04', 'CON-ADDICT-05'];
      const actualIds = MODULE_U4_CONCEPTS.map((c) => c.id);
      expect(actualIds).toEqual(expectedIds);

      const slugs = new Set(MODULE_U4_CONCEPTS.map((c) => c.slug));
      expect(slugs.size).toBe(5);
    });

    it('should enforce zero exam mappings (pure universal knowledge track)', () => {
      for (const concept of MODULE_U4_CONCEPTS) {
        expect(concept.examMappings).toEqual([]);
      }
    });

    it('should verify each concept has at least 4 claims with authoritative clinical locators', () => {
      let totalClaims = 0;
      for (const concept of MODULE_U4_CONCEPTS) {
  ...
  describe('Database Persistence & Relationship Verification', () => {
    it('should verify that all 5 concepts exist under the World Understanding domain in SQLite', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'neurobiology-of-addiction-and-clinical-recovery' },
        include: {
          domain: true,
          topics: {
            include: {
      ...
      for (const concept of allConcepts) {
        ...
        for (const cl of concept.claims) {
          ...
          expect(cl.evidence[0].sourceId).toBe('SRC-YALE-ADDICTION-TREATMENT');
        }
    ...
    it('should verify total claims, blocks, units, and questions count in database for Module U4', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'neurobiology-of-addiction-and-clinical-recovery' },
      });
    ...
    it('should confirm strict isolation: universal addiction medicine concepts do not leak into exam lenses', async () => {
      const examMappings = await db.examConceptMapping.findMany({
        where: {
          concept: {
            id: {
              in: ['CON-ADDICT-01', 'CON-ADDICT-02', 'CON-ADDICT-03', 'CON-ADDICT-04', 'CON-ADDICT-05'],
            },
          },
        },
      });
