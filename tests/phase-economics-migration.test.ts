...
describe('Phase E: Indian Economy & Macroeconomics Migration Audit', () => {
  ...
  it('should verify canonical concepts across Batch E1, E2, E3, E4, and E5', () => {
    const totalConcepts = BATCH_E1_CONCEPTS.length + BATCH_E2_CONCEPTS.length + BATCH_E3_CONCEPTS.length + BATCH_E4_CONCEPTS.length + BATCH_E5_CONCEPTS.length;
    ...
    expect(totalConcepts).toBe(106);
  });

  it('should verify every concept contains claims, content blocks, and exam mappings', () => {
    const allConcepts = [...BATCH_E1_CONCEPTS, ...BATCH_E2_CONCEPTS, ...BATCH_E3_CONCEPTS, ...BATCH_E4_CONCEPTS, ...BATCH_E5_CONCEPTS];
  ...
  it('should verify database contains all 106 active Economics concepts', async () => {
    ...
    expect(subject.topics.length).toBeGreaterThanOrEqual(44);
    const totalDbConcepts = subject.topics.reduce((acc, t) => acc + t.concepts.length, 0);
    expect(totalDbConcepts).toBeGreaterThanOrEqual(106);
  });
});
