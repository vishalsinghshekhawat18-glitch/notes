...
import { MODULE_U1_CONCEPTS, seedModuleU1CosmicDynamics } from '../lib/benchmark/batch-u1-cosmic-dynamics-seed';

describe('Universal Knowledge & World Understanding — Module U1: Space Exploration, Rocketry & Cosmic Dynamics', () => {
  beforeAll(async () => {
    await seedModuleU1CosmicDynamics();
  });

  describe('Structural & Quantitative Verification', () => {
    it('should contain exactly 5 canonical concepts under Topic 1', () => {
      expect(MODULE_U1_CONCEPTS.length).toBe(5);

      const topicOrders = new Set(MODULE_U1_CONCEPTS.map((c) => c.topicOrder));
      expect(topicOrders.size).toBe(1);
      expect(topicOrders.has(1)).toBe(true);

      const expectedIds = ['CON-COSMIC-01', 'CON-COSMIC-02', 'CON-COSMIC-03', 'CON-COSMIC-04', 'CON-COSMIC-05'];
      const actualIds = MODULE_U1_CONCEPTS.map((c) => c.id);
      expect(actualIds).toEqual(expectedIds);

      // Verify unique slugs
      const slugs = new Set(MODULE_U1_CONCEPTS.map((c) => c.slug));
      expect(slugs.size).toBe(5);
    });

    it('should enforce zero exam mappings (pure universal knowledge track)', () => {
      for (const concept of MODULE_U1_CONCEPTS) {
  ...
  describe('Database Persistence & Relationship Verification', () => {
    it('should verify that all 5 concepts exist under the World Understanding domain in SQLite', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'space-exploration-rocketry-cosmic-dynamics' },
        include: {
          domain: true,
          topics: {
            include: {
