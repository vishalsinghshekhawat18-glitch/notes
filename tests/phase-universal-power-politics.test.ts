...
import { MODULE_U6_CONCEPTS, seedModuleU6PowerPolitics } from '../lib/benchmark/batch-u6-power-politics-seed';

describe('Universal Knowledge & World Understanding — Module U6: Power, Politics & The Post-Cold War World', () => {
  beforeAll(async () => {
    await seedModuleU6PowerPolitics();
  });

  describe('Structural & Quantitative Verification', () => {
    it('should contain exactly 5 canonical concepts under Topic 1', () => {
      expect(MODULE_U6_CONCEPTS.length).toBe(5);

      const topicOrders = new Set(MODULE_U6_CONCEPTS.map((c) => c.topicOrder));
      expect(topicOrders.size).toBe(1);
      expect(topicOrders.has(1)).toBe(true);

      const expectedIds = ['CON-POL-01', 'CON-POL-02', 'CON-POL-03', 'CON-POL-04', 'CON-POL-05'];
      const actualIds = MODULE_U6_CONCEPTS.map((c) => c.id);
      expect(actualIds).toEqual(expectedIds);

      const slugs = new Set(MODULE_U6_CONCEPTS.map((c) => c.slug));
      expect(slugs.size).toBe(5);
    });

    it('should enforce zero exam mappings (pure universal knowledge track)', () => {
      for (const concept of MODULE_U6_CONCEPTS) {
        expect(concept.examMappings).toEqual([]);
      }
    });

    it('should verify each concept has at least 4 claims with authoritative scientific/academic locators', () => {
      let totalClaims = 0;
      for (const concept of MODULE_U6_CONCEPTS) {
  ...
  describe('Database Persistence & Relationship Verification', () => {
    it('should verify that all 5 concepts exist under the World Understanding domain in SQLite', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'power-politics-and-post-cold-war-order' },
        include: {
          domain: true,
          topics: {
            include: {
      ...
      for (const concept of allConcepts) {
        ...
        for (const cl of concept.claims) {
          ...
          expect(cl.evidence[0].sourceId).toBe('SRC-YALE-POWER-AND-POLITICS');
        }
    ...
    it('should verify total claims, blocks, units, and questions count in database for Module U6', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'power-politics-and-post-cold-war-order' },
      });
    ...
    it('should confirm strict isolation: universal power & politics concepts do not leak into exam lenses', async () => {
      const examMappings = await db.examConceptMapping.findMany({
        where: {
          concept: {
            id: {
              in: ['CON-POL-01', 'CON-POL-02', 'CON-POL-03', 'CON-POL-04', 'CON-POL-05'],
            },
          },
        },
      });
