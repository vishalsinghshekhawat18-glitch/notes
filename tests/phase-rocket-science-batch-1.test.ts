...
import { BATCH_R1_CONCEPTS, seedBatchR1RocketScienceKnowledge } from '../lib/benchmark/batch-r1-rocket-science-seed';

describe('Rocket Science & Space Technology — Batch R1: Orbital Foundations, The Kármán Line, LEO/MEO/GEO, GNSS & Space Debris', () => {
  beforeAll(async () => {
    await seedBatchR1RocketScienceKnowledge();
  });

  describe('Structural & Quantitative Verification', () => {
    it('should contain exactly 5 canonical concepts across Topics 1 and 2', () => {
      expect(BATCH_R1_CONCEPTS.length).toBe(5);

      const topicOrders = new Set(BATCH_R1_CONCEPTS.map((c) => c.topicOrder));
      expect(topicOrders.size).toBe(2);
      expect(topicOrders.has(1)).toBe(true);
      expect(topicOrders.has(2)).toBe(true);

      const expectedIds = ['CON-SPACE-01', 'CON-SPACE-02', 'CON-SPACE-03', 'CON-SPACE-04', 'CON-SPACE-05'];
      const actualIds = BATCH_R1_CONCEPTS.map((c) => c.id);
      expect(actualIds).toEqual(expectedIds);

      // Verify unique slugs
      const slugs = new Set(BATCH_R1_CONCEPTS.map((c) => c.slug));
      expect(slugs.size).toBe(5);
    });

    it('should verify each concept has at least 4 claims with authoritative lecture locators', () => {
      let totalClaims = 0;
      for (const concept of BATCH_R1_CONCEPTS) {
        ...
        for (const claim of concept.claims) {
          ...
          expect(claim.locator).toContain('Rocket Science for Everyone with Marla Geha');
    ...
    it('should verify each concept has exactly 3 exam mappings for UPSC_GS3, RPSC_RAS, and GENERAL_TECH_LITERACY', () => {
      let totalMappings = 0;
      for (const concept of BATCH_R1_CONCEPTS) {
        expect(concept.examMappings.length).toBe(3);
        totalMappings += concept.examMappings.length;
        const examCodes = concept.examMappings.map((em) => em.examCode);
        expect(examCodes).toContain('UPSC_GS3');
        expect(examCodes).toContain('RPSC_RAS');
        expect(examCodes).toContain('GENERAL_TECH_LITERACY');

        for (const em of concept.examMappings) {
          expect(em.syllabusUnit.length).toBeGreaterThan(10);
          expect(em.notes.length).toBeGreaterThan(15);
          expect(['HIGH', 'MEDIUM', 'LOW']).toContain(em.priority);
  ...
  describe('Database Persistence & Relationship Verification', () => {
    it('should verify that all 5 concepts and their relational children exist in SQLite', async () => {
      const subject = await db.subject.findFirst({
        where: { slug: 'aerospace-orbital-mechanics-rocket-science' },
        include: {
          topics: {
            include: {
              concepts: {
      ...
      expect(subject?.topics.length).toBe(2);
      ...
      for (const concept of allConcepts) {
        ...
        for (const cl of concept.claims) {
          ...
          expect(cl.evidence[0].sourceId).toBe('SRC-YALE-ROCKET-SCIENCE');
        }
    ...
    it('should verify exam mappings point to valid exams in the database', async () => {
      const exams = await db.exam.findMany({
        where: {
          slug: {
            in: ['upsc-gs3', 'rpsc-ras', 'general-tech-literacy'],
          },
        },
      });
  ...
  describe('Pedagogy & Cognitive Integrity', () => {
    it('should maintain strict character bounds for rapid recall revision units', () => {
      for (const concept of BATCH_R1_CONCEPTS) {
        const flash = concept.revisionUnits.find((u) => u.type === 'FLASH_30S')!;
        expect(flash.content.length).toBeLessThanOrEqual(350);
        expect(flash.content).toMatch(/!/); // Must have punchy exclamation

        const summary = concept.revisionUnits.find((u) => u.type === 'SUMMARY_2M')!;
        expect(summary.content.length).toBeGreaterThanOrEqual(150);

        const arch = concept.revisionUnits.find((u) => u.type === 'ARCHITECTURE_5M')!;
        expect(arch.content).toContain('1.');
        expect(arch.content).toContain('2.');
      }
    });

    it('should verify question options are distinct and plausible', () => {
      for (const concept of BATCH_R1_CONCEPTS) {
        for (const q of concept.questions) {
